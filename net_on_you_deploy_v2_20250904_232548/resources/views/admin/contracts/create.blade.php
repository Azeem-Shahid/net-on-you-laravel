...
<div class="container mx-auto px-4">
    <div class="bg-white shadow-lg rounded-lg overflow-hidden">
        ...
        <div class="p-6">
            <form action="{{ route('admin.contracts.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select id="language" name="language" required
                            ...
                            @foreach($languages as $language)
                                <option value="{{ $language->code }}" {{ old('language') == $language->code ? 'selected' : '' }}>
                                    {{ $language->name }}
                                    @if($language->is_core)
                                        <span class="text-blue-600">(Core)</span>
                                    @endif
                                </option>
                            @endforeach
                        </select>
                        <p class="mt-1 text-sm text-gray-500">
                            ...
                            Core languages have full content support. Other languages will fallback to English.
                        </p>
                        @error('language')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
