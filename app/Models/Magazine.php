...
class Magazine extends Model
{
    ...
    /**
     * Get magazines by language with fallback to English
     * Uses core language system for proper fallback
     */
    public static function getByLanguageWithFallback(string $languageCode, int $limit = null)
    {
        // Try to get magazines in requested language first
        $magazines = static::active()->published()->byLanguage($languageCode)->get();
        
        if ($magazines->count() === 0) {
            // Get fallback language using the core language system
            $fallbackLanguage = Language::getContentFallbackLanguage($languageCode);
            
            if ($fallbackLanguage !== $languageCode) {
                $magazines = static::active()->published()->byLanguage($fallbackLanguage)->get();
            }
        }
        
        if ($limit) {
            $magazines = $magazines->take($limit);
        }
        
        return $magazines->sortByDesc('published_at');
    }

    /**
     * Get the latest magazine for a specific language with fallback
     */
    public static function getLatestByLanguageWithFallback(string $languageCode)
    {
        $magazines = static::getByLanguageWithFallback($languageCode, 1);
