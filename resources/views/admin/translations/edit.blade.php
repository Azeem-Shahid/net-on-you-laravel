...
<div class="container mx-auto px-4 py-6">
    <div class="max-w-2xl mx-auto">
        <div class="mb-6">
            ...
            <p class="text-gray-600 mt-1">Update translation key and value</p>
        </div>
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <select name="language_code" 
                            ...
                            @foreach($languages as $language)
                                <option value="{{ $language->code }}" {{ old('language_code', $translation->language_code) == $language->code ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <textarea name="value" 
                                  ...
                                  required>{{ old('value', $translation->value) }}</textarea>
                    ...
                    <div>
                        ...
                        <select name="module" 
                            ...
                            @foreach($modules as $module)
                                <option value="{{ $module }}" {{ old('module', $translation->module) == $module ? 'selected' : '' }}>
                                    {{ ucfirst($module) }}
                                </option>
                            @endforeach
                            <option value="custom" {{ old('module', $translation->module) == 'custom' ? 'selected' : '' }}>Custom</option>
                        </select>
                    ...
                    <div id="customModuleField" class="{{ old('module', $translation->module) == 'custom' ? '' : 'hidden' }}">
                        ...
                        <input type="text" 
                               name="custom_module" 
                               id="custom_module" 
                               value="{{ old('custom_module', $translation->module) }}"
                               class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-[#1d003f] focus:border-[#1d003f]"
                    ...
                    <div class="bg-gray-50 border border-gray-200 rounded-md p-4">
                        <div class="flex">
                            ...
                            <div class="ml-3">
                                <h3 class="text-sm font-medium text-gray-800">Translation Information</h3>
                                <div class="mt-2 text-sm text-gray-700">
                                    <p><strong>Created:</strong> {{ $translation->created_at->format('M d, Y H:i') }}</p>
                                    <p><strong>Last Updated:</strong> {{ $translation->updated_at->format('M d, Y H:i') }}</p>
                                    <p><strong>Created By:</strong> {{ $translation->createdByAdmin->name ?? 'Unknown' }}</p>
                                    <p><strong>Last Updated By:</strong> {{ $translation->updatedByAdmin->name ?? 'Unknown' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
...
// Initialize custom module field if needed
